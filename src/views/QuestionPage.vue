...
answers.value = schuffle(answers.value)
...
<div v-if="question" class="flex h-full w-full flex-col items-center gap-8 p-10">
  <BaseTitle>{{ question.category }} - <MainScore/></BaseTitle>
  <!-- {{ question.question }} -->

  <div v-html="question.question" class="text-center text-2xl font-bold"></div>
    <div class="grid w-full flex-grow grid-cols-2 gap-8">
      <div v-for="answer in answers" v-html="answer.answer" :key="answer.id" class="bg-green-500 flex items-center justify-center text-4xl rounded-lg text-white py-10 px-2 "></div>
    </div>
